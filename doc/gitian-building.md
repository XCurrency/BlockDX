...
*Setup instructions for a gitian build of xc3 using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the xc3
...
Clone the git repositories for xc3dx and gitian and then checkout the xc3dx version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/xc3dx-crypto/xc3dx.git
cd xc3dx
...
Follow the instructions in [doc/release-process.md](release-process.md) in the xc3dx repository
...
To build xc3 (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the xc3dx repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/xc3dx/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/xc3dx-crypto/xc3dx
...
For example:
```bash
URL=https://github.com/crowning-/xc3dx.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit xc3dx=${COMMIT} --url xc3dx=${URL} ../xc3dx/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xc3dx=${COMMIT} --url xc3dx=${URL} ../xc3dx/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xc3dx=${COMMIT} --url xc3dx=${URL} ../xc3dx/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/xc3dx-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/xc3dx-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/xc3dx-build.assert
```
...
[xc3dx/gitian.sigs](https://github.com/xc3dx-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
