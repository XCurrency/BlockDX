...
ClientModel::ClientModel(OptionsModel *optionsModel, QObject *parent) : QObject(parent),
                                                                        ...
                                                                        cachedReindexing(false), cachedImporting(0),
...
QString ClientModel::getServicenodeCountString() const {
    return tr("Total: %1 (OBF compatible: %2 / Enabled: %3)").
            arg(QString::number((int) mnodeman.size())).arg(
            QString::number((int) mnodeman.CountEnabled(ActiveProtocol()))).
...
void ClientModel::updateTimer() {
    ...
    if (cachedNumBlocks != newNumBlocks ||
        ...
        servicenodeSync.RequestedServicenodeAttempt != prevAttempt ||
        servicenodeSync.RequestedServicenodeAssets != prevAssets) {
