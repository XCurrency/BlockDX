...
void OverviewPage::setBalance(const CAmount &balance, const CAmount &unconfirmedBalance, const CAmount &immatureBalance,
    ...
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance, false,
...
//    ui->labelAnonymized->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, anonymizedBalance, false, BitcoinUnits::separatorAlways));
    ui->labelTotal->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance + unconfirmedBalance, false,
                                                            BitcoinUnits::separatorAlways));
    ...
    ui->labelWatchTotal->setText(
            BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance,
...
//        connect(ui->toggleObfuscation, SIGNAL(clicked()), this, SLOT(toggleObfuscation()));
    ...
    // update the display unit, to not use the default ("XCurrency")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress() {
    ...
    QString strAnonymizexc3Amount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit,
                                                                            nAnonymizexc3Amount * COIN, false,
                                                                            BitcoinUnits::separatorAlways);
...
//        ui->obfuscationProgress->setToolTip(tr("No inputs detected"));
        ...
        strAnonymizexc3Amount = strAnonymizexc3Amount.remove(strAnonymizexc3Amount.indexOf("."),
                                                                           BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizexc3Amount + " / " + tr("%n Rounds", "", nObfuscationRounds);
...
//        ui->labelAmountRounds->setText(strAmountAndRounds);
    ...
    if (nMaxToAnonymize > nAnonymizexc3Amount * COIN) nMaxToAnonymize = nAnonymizexc3Amount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizexc3Amount * COIN) {
//        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
//                                              .arg(strAnonymizexc3Amount));
        strAnonymizexc3Amount = strAnonymizexc3Amount.remove(strAnonymizexc3Amount.indexOf("."),
                                                                           BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizexc3Amount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::obfuScationStatus() {
    ...
    if (((nBestHeight - obfuScationPool.cachedNumBlocks) / (GetTimeMillis() - nLastDSProgressBlockTime + 1) > 1))
        return;
