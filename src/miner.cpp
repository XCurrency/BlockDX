...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("xc3Miner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("xc3Miner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("xc3Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xc3-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (chainActive.Tip()->nTime < 1471482000 || vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || nReserveBalance >= pwallet->GetBalance() || !servicenodeSync.IsSynced()) {
        ...
        LogPrintf("Running xc3Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
